...
def test_single_client_single_column_convergence():
    ...
    client_1 = Client(name = "1", mode = "keras")
    client_1.add_data(new_X = data_X, new_Y = data_Y)

    server = Server(
        mode = "keras",
        participationRatio = 1,
        initialWeights = get_initial_weights(mode = "keras", columnIdx = 3),
        clients = [client_1]
    )
...
def test_multi_client_single_column_convergence():
    ...
    client_1 = Client(name = "1", mode = "keras")
    client_2 = Client(name = "2", mode = "keras")
    client_3 = Client(name = "3", mode = "keras")
...
def test_single_client_multi_column_convergence():
    ...
    server = Server(
        mode = "keras",
        participationRatio = 1,
        initialWeights = get_initial_weights(mode = "keras"),
        clients = [client_1]
    )
    server.start_clients()
...
def test_multi_client_multi_column_convergence():
    ...
    model = get_model(mode = "keras", inputDim = data_X.shape[1], outputDim = 1)
...
def test_similar_wb_client_server():
    ...
    w1, b1 = get_params(mode = "keras", model = client_1.get_model())
    w2, b2 = get_params(mode = "keras", model = client_2.get_model())
    w3, b3 = get_params(mode = "keras", model = client_3.get_model())
...
def test_participation_ratio():
    ...
    client_2 = Client(name = "2", mode = "keras")
    client_3 = Client(name = "3", mode = "keras")
    client_4 = Client(name = "4", mode = "keras")
...
def test_split_data_single_column_convergence():
    ...
    server = Server(
        mode = "keras",
        participationRatio = 1,
        initialWeights = get_initial_weights(mode = "keras", columnIdx = 1),
        clients = [client_1, client_2, client_3]
    )
